...
- `list_variables`.
- `get_variable`.
- Structured matrix and scalar serialization.
- Large output limits and binary transfer.
...
`bxEvalString` reports only success or failure, so PR7 responses do not capture
console output. More complete result serialization belongs to PR8.
