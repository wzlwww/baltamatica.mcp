...
- **下载预编译版**（推荐）：从 [GitHub Releases](https://github.com/wzlwww/baltamatica.mcp/releases) 下载对应平台的 `mcp_bridge.bex*`（macOS arm64 `mcp_bridge.bexmaci64`、Linux x86-64 `mcp_bridge.bexa64`；Windows 暂需自行编译），放进某目录即可。
...
- [x] 发布 **v0.2.0**：PyPI（`pip install baltamatica-mcp`）+ GitHub Release（含 macOS/Linux 预编译二进制）

### 下一步

- [ ] BEX 图形导出到文件：受限于北太天元本身缺少 `saveas`/`print`/`exportgraphics` 等函数，需厂商侧支持
- [ ] Windows 预编译二进制 `mcp_bridge.bexw64`（需 Windows + 北太天元编译环境）
